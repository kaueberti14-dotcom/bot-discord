...
CANAL_AVISOS_COMANDOS = "🚨│avisos-comandos"
...
async def on_message(msg):
    ...
    # EQUIPE
        ...
        if canal_equipes is None:
            return await msg.reply("Não achei o canal de equipes.")
    ...
    # VS
        ...
        embed.add_field(
            ...
            value=(
                f"🏆 Vitórias: `{p1.get('wins', 0)}`\n"
                f"💀 Derrotas: `{p1.get('losses', 0)}`\n"
                f"🔥 Sequência: `{p1.get('streak', 0)}`"
            ),
            inline=True
        )
        ...
        embed.add_field(
            ...
            value=(
                f"🏆 Vitórias: `{p2.get('wins', 0)}`\n"
                f"💀 Derrotas: `{p2.get('losses', 0)}`\n"
                f"🔥 Sequência: `{p2.get('streak', 0)}`"
            ),
            inline=True
        )
    ...
    # BOT
        ...
        canal_avisos = get_channel(msg.guild, CANAL_AVISOS_COMANDOS)
    ...
    # REGRAS GERAIS
        ...
        embed.add_field(
            name="👥│Respeito aos Membros do Servidor",
            value="Todos os membros devem ser tratados com respeito. Não serão toleradas ofensas, bullying ou qualquer forma de discriminação.",
            inline=False
        )

        embed.add_field(
            name="🚫│Preconceito",
            value="Atitudes como racismo, xenofobia ou qualquer outro tipo de preconceito resultarão em banimento imediato.",
            inline=False
        )

        embed.add_field(
            name="📛│Proibição de Spam",
            value="É vedado o envio repetitivo e intencional de mensagens, caracterizando spam.",
            inline=False
        )

        embed.add_field(
            name="🔞│Conteúdo Impróprio",
            value="Não é permitido compartilhar conteúdos inadequados, como pornografia ou materiais ofensivos.",
            inline=False
        )

        embed.add_field(
            name="📜│Diretrizes do Discord",
            value="Todos os membros devem seguir as diretrizes oficiais do Discord, além das regras deste servidor.",
            inline=False
        )

        embed.add_field(
            name="⚖️│Conduta em Relação a Outros Membros",
            value="Não é permitido desmerecer ou ridicularizar outros membros, especialmente por desempenho em partidas.",
            inline=False
        )

        embed.add_field(
            name="⚠️│Observação",
            value="Serão toleradas, no máximo, três ocorrências de reclamações relacionadas a ofensas direcionadas à habilidade de outros membros. Após isso, o membro será expulso do servidor.",
            inline=False
        )
    ...
    # HOST / REGRAS RANQUEADAS
        ...
        await canal_regras.send(
            embed=pages[0],
            file=file,
            view=RegrasView(pages)
        )
